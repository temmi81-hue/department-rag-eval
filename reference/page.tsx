...
export default function Home() {
  ...
  function submit(event: FormEvent) { event.preventDefault(); setDraft(''); if (!question.trim()) { setError('업무 상황을 입력해 주세요.'); return; } setError(''); setWorkflowTab('check'); setActiveTab('regulatory-search'); setSearched(true); void searchRag(); }
  function navigateTab(id: string, target: string) { setActiveTab(id); if (id === 'dashboard') { setSearched(false); setDraft(''); setRag(null); setError(''); setRequestClosed(false); setRequestStatus('작성중'); setRequestId(null); setWorkflowTab('input'); } if (id === 'compliance-hub') void loadComplianceEntries(); window.setTimeout(() => { const element = document.querySelector(target) ?? document.querySelector('#regulatory-search'); element?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 0); }
  function navigateWorkflow(id: string, target: string) { setWorkflowTab(id); if (id === 'input') { setActiveTab('dashboard'); setSearched(false); setDraft(''); setRag(null); setError(''); setRequestClosed(false); setRequestStatus('작성중'); setRequestId(null); } window.setTimeout(() => { const element = document.querySelector(target) ?? document.querySelector('#regulatory-search'); element?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 0); }
  ...
  function confirmDraft() { if (!pendingDepartment) return; const department = pendingDepartment; const recipientIds = selectedDepartments.includes(department.id) ? selectedDepartments : [...selectedDepartments, department.id]; const recipientLines = recipientIds.map((id) => { const selected = results.find((item) => item.id === id); return `${selected?.name ?? id} <${departmentEmail(id)}>`; }).join(', '); setSelectedDepartments(recipientIds); const requestItems = ['관련 절차와 사전 검토 필요 여부를 확인해 주세요.', ...(recipientIds.length > 1 ? ['여러 부서가 관련되어 있는데, 어느 부서에 먼저 협의를 요청하면 될지 처리 순서를 안내해 주세요.'] : ['추가로 협의해야 할 부서나 담당 업무가 있으면 안내해 주세요.']), '사전 협의 시 첨부해야 할 자료(예: 투자품의서, 견적서, 타당성 검토안)가 있다면 안내해 주세요.', '검토 및 회신까지 예상되는 소요 기간을 알려주세요.'].map((item, index) => `${index + 1}. ${item}`).join('\n'); setDraft(`수신: ${recipientLines}\n제목: [사전 협의 요청] ${department.name} 관련 업무 검토\n\n업무 상황\n${question}\n\n사업장: ${site === '미선택' ? '미입력' : site}\n요청 사항\n${requestItems}\n\n추천 근거\n${department.reasons.join(', ')}\n업무분장 근거\n${department.sourcePath}`); setPendingDepartment(null); setRequestStatus('작성중'); }
...
function RagPanel({ result, fallback }: { result: RagResult; fallback: Match[] }) {
